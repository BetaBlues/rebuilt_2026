    ...
    public MiddleCamera() {
     ...
     public void estimateMiddlePose(Field2d field, ChassisGyro gyro) {
          ...
          try {
               ...
               SmartDashboard.putNumber("Middle Camera Distance x", m_robotPose.getX());
               SmartDashboard.putNumber("Middle Camera Distance y", m_robotPose.getY());
               SmartDashboard.putNumber("Middle Camera Distance z", m_robotPose.getZ());
               SmartDashboard.putNumber("Middle Camera Distance to tag", m_target.getBestCameraToTarget().getX());
               SmartDashboard.putNumber("Middle Camera Angle to tag", m_target.getBestCameraToTarget().getY());
               SmartDashboard.putNumber("Middle Camera Delta to tag", m_target.getBestCameraToTarget().getZ());
          }

          catch(Exception e) {
               SmartDashboard.putBoolean("Middle Camera Has Target:", isIndex);
          }
     ...
     public void showData() {
          ...
          SmartDashboard.putBoolean("Middle Camera Connected", MiddleCamera.isConnected());
          SmartDashboard.putNumber("Middle Cam Apriltag Id", m_apriltagId);
          // SmartDashboard.put
     } 
}
